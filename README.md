...
- **多部件采集**：支持 CPU、内存、硬盘、GPU、NPU、网卡、机箱等部件指标采集，共 159 个指标
- **来源层架构**：`internal/source/` 抽象数据获取与解析（14 个包：proc/sys/ipmi/lscpu/mce/dmesg/dmidecode/statfs/smartctl + dcmi/npu_smi/hccn_tool/nvidia_smi），全部 7 个采集器已接入来源层，采集器不再直接读文件或执行命令，来源返回 parsed struct + 单例 + 可注入 fetcher + 缓存
...
- **特性层架构**（v0.3.0 新增）：`features/` 承载基于采集基础能力构建的上层模块——`features/health`（健康度评估，消费 `collector.Metric`，按部件评估器 + 局部 scheme）、`features/web`（Web 仪表盘）、`features/dfee`（能效监控，25 张实时图表 + CPU 利用率推导，v0.3.1 新增）
...
共 159 个指标，覆盖 7 个部件。详见 [指标清单](docs/CATMonitor_indi_list.md)。
...
| Network | 5 | 1 | 3 | 1 | ✅ | ✅ (5/5) |
| Chassis | 5 | 2 | 3 | 0 | ✅ | ✗ (Linux 专有，依赖 ipmitool SDR) |
...
> v0.3.0 引入指标采集目录（`configs/metrics.yaml` + 模块覆盖）与 `features/` 特性层（health/web），健康度抽取为 `features/health` 按部件评估器，Web 迁至 `features/web`。
> v0.3.1 新增 Chassis 机箱环境采集器（5 指标，ipmitool SDR）、Disk 读/写耗时（read_latency/write_latency）、`features/dfee` 能效监控模块（25 张实时图表 + CPU 利用率推导）。指标总数 152→159，部件 6→7。
...
│   │   └── chassis/         #   chassis.go (5 指标: 功耗/进出风温度/风扇, ipmitool SDR, v0.3.1 新增)
...
│   ├── config/              # 配置管理
│   └── storage/             # 数据存储（JSONL）
├── features/                # 特性层：基于采集基础能力构建的上层模块（health/web/dfee 等）
...
│   │   ├── server.go        #     HTTP 路由与处理函数（含 dfee.Register）
...
│       ├── filter.go        #     能效指标过滤（74 项从 159 中筛选）
