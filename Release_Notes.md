...
- 新增独立 `features/stress`，通过 `catmonitor stress` 显式运行 STREAM、HPL、HPCG、Ascend NPU Burn；普通 health 和 daemon 不自动触发。
...
- 新增 Ascend NPU Burn 管理员镜像构建工具：默认使用仓库固定源码和管理员批准的本地 CANN/torch_npu 基础镜像；A3 默认使用无补丁 profile，可选兼容补丁只作用于隔离快照；构建器使用 Bash 显式发现并 source CANN 环境，在 wheel 前校验 HAL、torch、torch_npu 和 TBE，再以离线强制重装方式安装本轮 wheel；构建不要求 driver mount、`npu-smi` 或 NPU 设备，也不运行 NPU 负载；manifest 追溯所选 CANN 环境、wheel 哈希/安装位置和各项预检结果。
