...
func parseBenchmark(name, output, resultDir string) (map[string]float64, string, error) {
	switch name {
	case "stream":
	...
	case "hpcg":
		return parseHPCG(output, resultDir)
	default:
...
func parseHPL(output string) (map[string]float64, string, error) {
	...
	for _, line := range strings.Split(output, "\n") {
		...
		if err1 == nil && err2 == nil {
			return map[string]float64{"time_seconds": timeValue, "gflops": gflops}, "stdout", nil
		}
	}
	return nil, "", fmt.Errorf("HPL Time/Gflops row not found")
}

func parseHPCG(output, resultDir string) (map[string]float64, string, error) {
	if values, ok := parseHPCGText(output); ok {
		return values, "stdout", nil
	}
	path, err := latestHPCGResult(resultDir)
	if err != nil {
		return nil, "", err
	}
...
func latestHPCGResult(dir string) (string, error) {
	entries, err := os.ReadDir(dir)
	if err != nil {
		return "", fmt.Errorf("HPCG result directory unavailable: %w", err)
	}
	...
	for _, entry := range entries {
		name := entry.Name()
		if entry.IsDir() || !strings.HasSuffix(strings.ToLower(name), ".txt") ||
			!(strings.HasPrefix(strings.ToLower(name), "hpcg") || strings.HasPrefix(name, "HPCG-Benchmark")) {
			continue
		}
		info, err := entry.Info()
		if err == nil {
			candidates = append(candidates, candidate{filepath.Join(dir, name), info.ModTime()})
		}
	}
	if len(candidates) == 0 {
		return "", fmt.Errorf("no HPCG-Benchmark*.txt result found in %q", dir)
	}
